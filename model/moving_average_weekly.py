...
def moving_average_model_weekly(prod, cus_no, mat_no, post_outlier_period_flag, baseline=False, **kwargs):
    ...
    output_error = pd.DataFrame(data=[[cus_no, mat_no, rmse, mape, mae,
                                       ...
                                       np.nanmedian(np.absolute(np.array(output_result.rolling_24week_percent_error))),
                                       np.nanmax(np.absolute(np.array(output_result.rolling_24week_percent_error))),
                                       np.nanmean(np.absolute(np.array(output_result.rolling_24week_percent_error))),
                                       np.nanmean(np.absolute(np.array(output_result.rolling_24week_quantity))),
                                       np.nanmedian(np.absolute(np.array(output_result.rolling_48week_percent_error))),
                                       np.nanmax(np.absolute(np.array(output_result.rolling_48week_percent_error))),
                                       np.nanmean(np.absolute(np.array(output_result.rolling_48week_percent_error))),
                                       np.nanmean(np.absolute(np.array(output_result.rolling_48week_quantity))),
