...
def moving_average_model_monthly(prod, cus_no, mat_no, baseline = False, **kwargs):
    # min_train_days must be defined for it to be used as baseline****
    ...
    output_error = pd.DataFrame(data=[[cus_no, mat_no, rmse, mape,
                                       ...
                                       np.nanmean(np.absolute(np.array(output_result.rolling_4month_percent_error))),
                                       ...
                                       np.nanmean(np.absolute(np.array(output_result.rolling_6month_percent_error))),
                                       np.nanmean(np.absolute(np.array(output_result.rolling_6month_quantity))),
                                       ...
                                       np.nanmean(np.absolute(np.array(output_result.rolling_12month_percent_error))),
                                       np.nanmean(np.absolute(np.array(output_result.rolling_12month_quantity))),
                                ...
                                columns=['cus_no', 'mat_no', 'rmse', 'mape',
                                         ...
                                         'mre_med_12', 'mre_max_12', 'mre_mean_12', 'quantity_mean_12',
