    ...
    try:
        if(param.get('yearly_seasonality') == True):
            ...
            while (len(rem_data.ds) >= test_points):
                # prophet
                m = Prophet(weekly_seasonality=False, holidays=holidays, yearly_seasonality=yearly_seasonality, changepoint_prior_scale=changepoint_prior_scale, seasonality_prior_scale=seasonality_prior_scale)
                m.fit(train);
            ...
            while (len(rem_data.ds) >= test_points):
                # prophet
                m = Prophet(weekly_seasonality=False, holidays=holidays, yearly_seasonality=yearly_seasonality, changepoint_prior_scale=changepoint_prior_scale)
                m.fit(train);
