    ...
    if (param.get('yearly_seasonality') == True):
        ...
        output_error = pd.DataFrame(
            data=[[cus_no, mat_no, rmse_calculator(output_result.y_Prophet, output_result.y),
                   ...
                   np.nanmedian(np.absolute(np.array(output_result.rolling_4week_percent_error_prophet))),
                   np.nanmax(
                       np.absolute(np.array(output_result.rolling_4week_percent_error_prophet))),
                   np.nanmean(np.absolute(np.array(output_result.rolling_4week_percent_error_prophet))),
                   np.nanmean(np.absolute(np.array(output_result.rolling_4week_quantity))),
