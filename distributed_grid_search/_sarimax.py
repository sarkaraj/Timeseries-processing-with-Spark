...
def _get_pred_dates(last_ts_date, pred_points):
    import pandas as pd
    pred_dates = pd.DataFrame({'ds': [last_ts_date]})
    for i in range(pred_points):
        pred_dates = pred_dates.append({'ds': pred_dates.ds[i] + pd.DateOffset(days = 7)}, ignore_index=True)
    return pred_dates.loc[1:,].reset_index(drop = True)
...
def sarimax(cus_no, mat_no, pdq, seasonal_pdq, prod, **kwargs):
    ...
    # Transform data to include holidays as dummy variables
    prod = generate_sarimax_holiday_input_data(ts_date_df=prod)
    prod = prod.fillna(0)

    # if holiday_effect == True:
    # Generate holidays data set for prediction points
    exog_prediction = generate_sarimax_holiday_input_data(ts_date_df= _get_pred_dates(last_ts_date=np.amax(prod.ds),
                                                                                      pred_points=pred_points))
    exog_prediction = exog_prediction.set_index('ds', drop = True)
    exog_prediction = exog_prediction.fillna(0)
    exog_prediction['const'] = 1.0
    ...
    while (len(test) > 0):
        ...
        exog_train = train_arima.drop('y', axis = 1)
        exog_train['const'] = 1.0

        exog_test = test_arima.drop('y', axis = 1)
        exog_test['const'] = 1.0
    ...
    endog_prod_arima = prod_arima.loc[:,'y']
    exog_prod_arima = sm.add_constant(prod_arima.drop('y', axis = 1))

    mod = sm.tsa.statespace.SARIMAX(endog = endog_prod_arima, exog = exog_prod_arima,order=pdq, seasonal_order=seasonal_pdq,
                                    enforce_stationarity=False, enforce_invertibility=False,
                                    measurement_error=False, time_varying_regression=False,
                                    mle_regression=True)
    ...
    pred_arima = results_arima.get_prediction(start=pd.to_datetime(np.amax(prod_arima.index)),
                                              ...
                                              exog = exog_prediction.values.astype('float'),
                                              dynamic=True)
...
if __name__ == '__main__':
    ...
    date = ["2018-05-24", "2018-05-31"]

    df = pd.DataFrame({'ds' : date})
        # .ds.apply(parser.parse)
    df.ds = df.ds.apply(parser.parse)
    print(type(np.amax(df.ds)))


    df1 = _get_pred_dates(np.amax(df.ds),pred_points= 3)
