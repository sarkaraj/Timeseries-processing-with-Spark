def twitter_outlier_detection(data):
    ...
    data['timestamp'] = data['timestamp'].apply(lambda x: time.mktime(dt.datetime.strptime(x,'%Y-%m-%d %H:%M:%S').timetuple()))
    ...
    data.y = data.y.interpolate(method='akima',limit = 10, limit_direction='both')
    return(data)


def ensm_mod(prod,cus_no,mat_no, holidays, min_train_days=731,test_points=1,dir_name=None):
    import pandas as pd
    import numpy as np
    import pyflux as pf
    ...
    output_result['rolling_6week_error'] = pd.rolling_sum(output_result['Error'], window = 6, min_periods=6, freq=None, center=False, how=None,)
    output_result['rolling_6week_y'] = pd.rolling_sum(output_result['y'], window = 6, min_periods=6, freq=None, center=False, how=None,)
    output_result['rolling_6week_percent_error'] = output_result['rolling_6week_error']/ output_result['rolling_6week_y']*100

    output_result['rolling_12week_error'] = pd.rolling_sum(output_result['Error'], window = 12, min_periods=12, freq=None, center=False, how=None,)
    output_result['rolling_12week_y'] = pd.rolling_sum(output_result['y'], window = 12, min_periods=12, freq=None, center=False, how=None,)
    output_result['rolling_12week_percent_error'] = output_result['rolling_12week_error']/ output_result['rolling_12week_y']*100

    output_result['rolling_6week_error_arima'] = pd.rolling_sum(output_result['Error_arima'], window = 6, min_periods=6, freq=None, center=False, how=None,)
    output_result['rolling_6week_y'] = pd.rolling_sum(output_result['y'], window = 6, min_periods=6, freq=None, center=False, how=None,)
    output_result['rolling_6week_percent_error_arima'] = output_result['rolling_6week_error_arima']/ output_result['rolling_6week_y']*100

    output_result['rolling_12week_error_arima'] = pd.rolling_sum(output_result['Error_arima'], window = 12, min_periods=12, freq=None, center=False, how=None,)
    output_result['rolling_12week_y'] = pd.rolling_sum(output_result['y'], window = 12, min_periods=12, freq=None, center=False, how=None,)
    output_result['rolling_12week_percent_error_arima'] = output_result['rolling_12week_error_arima']/ output_result['rolling_12week_y']*100

    output_result['rolling_6week_error_prophet'] = pd.rolling_sum(output_result['Error_prophet'], window = 6, min_periods=6, freq=None, center=False, how=None,)
    output_result['rolling_6week_y'] = pd.rolling_sum(output_result['y'], window = 6, min_periods=6, freq=None, center=False, how=None,)
    output_result['rolling_6week_percent_error_prophet'] = output_result['rolling_6week_error_prophet']/ output_result['rolling_6week_y']*100

    output_result['rolling_12week_error_prophet'] = pd.rolling_sum(output_result['Error_prophet'], window = 12, min_periods=12, freq=None, center=False, how=None,)
    output_result['rolling_12week_y'] = pd.rolling_sum(output_result['y'], window = 12, min_periods=12, freq=None, center=False, how=None,)
    output_result['rolling_12week_percent_error_prophet'] = output_result['rolling_12week_error_prophet']/ output_result['rolling_12week_y']*100

    output = pd.DataFrame(data= [[cus_no,mat_no,np.nanmedian(output_result.rolling_6week_percent_error),
                                  np.nanmedian(output_result.rolling_6week_percent_error_prophet),np.nanmedian(output_result.rolling_6week_percent_error_arima),
                                  np.nanmedian(output_result.rolling_12week_percent_error),
                                  np.nanmedian(output_result.rolling_12week_percent_error_prophet),np.nanmedian(output_result.rolling_12week_percent_error_arima),
                                  np.nanmedian(output_result.Error_Cumsum)]],
                          columns= ['cus_no', 'mat_no','6week_rolling_error','6week_rolling_error_prophet','6week_rolling_error_arima',
